...
export default function TechnicalEquipmentMethodsPage() {
  ...
  const saveEquipment = async (e) => {
    ...
    try {
      const created = await technicalConfigService.createTestEquipment(eqForm);
      setOk('Equipo de prueba creado. Ahora puede asignarle métodos.');
  ...
  const saveMethod = async (e) => {
    e.preventDefault(); setError(''); setOk('');
    try {
      await technicalConfigService.createEquipmentMethod({ ...methodForm, equipo_prueba: selectedEquipmentId || methodForm.equipo_prueba });
      setOk('Método creado para el equipo seleccionado.');
      ...
      <div style={styles.shell}>
        ...
        <div style={styles.twoCols}>
          <section style={styles.card}>
            <h2 style={styles.cardTitle}>Nuevo equipo de prueba</h2>
            <form onSubmit={saveEquipment}>
              ...
              <button style={{ ...styles.button, width: '100%', marginTop: 16 }} type="submit">Crear equipo</button>
            </form>
          </section>
          <section style={styles.card}>
            <h2 style={styles.cardTitle}>Métodos del equipo</h2>
            <label style={styles.label}>Equipo</label><select style={styles.select} value={selectedEquipmentId} onChange={(e) => setSelectedEquipmentId(e.target.value)}><option value="">Seleccione...</option>{equipment.map((eq) => <option key={eq.id} value={eq.id}>{eq.codigo} · {eq.nombre}</option>)}</select>
            <form onSubmit={saveMethod}>
              ...
              <button style={{ ...styles.button, width: '100%', marginTop: 16 }} type="submit">Crear método</button>
            </form>
          </section>
        </div>
        <section style={{ ...styles.card, marginTop: 22 }}>
          <h2 style={styles.cardTitle}>{selectedEquipment ? `Métodos de ${selectedEquipment.nombre}` : 'Métodos registrados'}</h2>
          <div style={styles.tableWrap}><table style={styles.table}><thead><tr><th style={styles.th}>Método</th><th style={styles.th}>Norma</th><th style={styles.th}>Equipo</th><th style={styles.th}>Estado</th></tr></thead><tbody>{methods.map((m) => <tr key={m.id}><td style={styles.td}><strong>{m.codigo}</strong><br />{m.nombre}</td><td style={styles.td}>{m.norma_referencia || '-'}</td><td style={styles.td}>{m.equipo_prueba_info?.nombre || m.equipo_prueba}</td><td style={styles.td}>{m.activo ? <span style={styles.badge}>Activo</span> : <span style={styles.badge}>Inactivo</span>}</td></tr>)}{!methods.length ? <tr><td style={styles.td} colSpan={4}>No hay métodos para este equipo.</td></tr> : null}</tbody></table></div>
        </section>
      </div>
    </main>
  );
